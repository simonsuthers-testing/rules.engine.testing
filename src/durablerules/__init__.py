from durable.lang import ruleset, post, when_all, m, assert_fact, post_batch, count
...
# https://www.npmjs.com/package/durable/v/0.36.95

#############################################################################################################
# basic test using hard coded values

with ruleset("test"):
    # antecedent
    @when_all(m.ISIN == "World")
    def say_hello(c):
        print(f"Basic rule: ISIN is {c.m.ISIN}")

test_data = [{'ISIN': 'World'}, {'ISIN': 'World1'}]

for data in test_data:
    try:
        post('test', data)
    except:
        print(f"Basic rule: ISIN is not {data['ISIN']}")
...
with ruleset('animal'):
    ...
    @when_all((m.country == 'ca'))
    def frog(c):
        dict1 = {'ISIN': c.m.ISIN, 'exchange': 'toronto' }
        c.assert_fact(dict1)

    @when_all((m.country != 'ca'))
    def frog(c):
        c.assert_fact({'ISIN': c.m.ISIN, 'exchange': 'not known' })

    @when_all(+m.exchange)
    def output(c):
        print('Fact: {0} {1} {2}'.format(c.m.ISIN, c.m.country, c.m.exchange))

test_data = [{'ISIN': 'CA001', 'country': 'ca'}, {'ISIN': 'UK002', 'country': 'uk'}]

for data in test_data:
    try:
        assert_fact('animal', data)
    except:
        print(f"{data['ISIN']} errored")
